...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItemId) {
        await api.put(
          `${API_BASE}/${isBlog ? "blogs" : "events"}/${editingItemId}`,
          form,
          {
            headers: {
  ...
  const handleDelete = async (id, type) => {
    try {
      await api.delete(
        `${API_BASE}/${type === "blog" ? "blogs" : "events"}/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      <aside className="w-64 bg-white shadow-md h-full p-6">
        <h2 className="text-lg font-bold text-[#84281B] mb-6">
          Admin Dashboard
        </h2>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b">
                <th className="py-2 text-sm font-medium text-gray-700">
                  Gambar
                </th>
                <th className="py-2 text-sm font-medium text-gray-700">
                  Judul
          ...
          >
            <div className="bg-white w-full max-w-lg p-6 rounded-lg shadow-lg relative">
              <h2 className="text-lg font-bold mb-4">
                {editingItemId ? "Edit" : "Tambah"}{" "}
                {activeTab === "blog" ? "Blog" : "Event"}
              </h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 rounded bg-[#84281B] text-white"
                  >
                    Simpan
                  </button>
                </div>
